...
# colorizer.load_state_dict(torch.load('siggraph_pretrained.pth'))
...
img_orig = cv2.imread('./imgs/cup/test_img.JPEG')[:,:,::-1]
...
# img_rs = scipy.misc.imresize(img_orig, (H_proc, W_proc))/255.
...
# add a color point - comment out for automatic colorization
(in_ab, in_mask) = add_color_patch(in_ab, in_mask, ab=[-20,30], hw=[128,160], P=8)
...
# ***** RUN MODEL *****
# embed()
out_ab_norm = tens2np(colorizer.forward(np2tens(img_rs_l_norm), np2tens(in_ab_norm), np2tens(in_mask_norm)).data)
...
cv2.imwrite('./imgs/cup/out.png',out_orig_rgb[:,:,::-1])
