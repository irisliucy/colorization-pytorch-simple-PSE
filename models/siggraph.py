...
class SIGGRAPHGenerator(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, classes=529):
        ...
        model_class=[nn.Conv2d(256, classes, kernel_size=1, padding=0, dilation=1, stride=1, bias=use_bias),]
        ...
        self.samp = nn.AvgPool2d(kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, input_A, input_B, mask_B):
        ...
        # print(conv7_3.shape) # 1, 512, 34, 23
        # print(conv3_3.shape) # 1, 256, 68, 45
        # print(self.model8up(conv7_3).shape)  #(1, 256, 68, 46)
        # print(self.model3short8(conv3_3).shape) #(1, 256, 68, 45)
        # print(self.model8(conv7_3).shape)
        ...
        out_class = self.upsample4(self.softmax(self.model_class(conv8_3.detach())))
...
class KMeansGenerator(nn.Module):
    def __init__(self, input_nc, k, norm_layer=nn.BatchNorm2d, use_tanh=True):
        super(KMeansGenerator, self).__init__()
        self.input_nc = input_nc
        self.k = k
        use_bias = True

        model = [nn.Conv2d(input_nc, 64, kernel_size=1, bias=use_bias),]
        model += [nn.LeakyReLU(negative_slope=.2),]
        model += [norm_layer(64),]

        model += [nn.Conv2d(64, 64, kernel_size=1, bias=use_bias),]
        model += [nn.LeakyReLU(negative_slope=.2),]
        model += [norm_layer(64),]
    ...
    def forward(self, input_cl):
        return self.model(input_cl)
