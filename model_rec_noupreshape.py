...
class SIGGRAPHGenerator(nn.Module):
    ...
    def forward(self, input_A, input_B, mask_B):
        ...
        # out_class = self.upsample4(self.softmax(self.model_class(conv8_3)))
        out_logit = self.model_class(conv8_3)
        N,C,X,Y = out_logit.shape
        out_logit_reshape = out_logit.permute(0,2,3,1).reshape(N*X*Y,C)
        out_class_reshape = self.softmax(out_logit_reshape)
        out_class = out_class_reshape.reshape(N,X,Y,C).permute(0,3,1,2)
